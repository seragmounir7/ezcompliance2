...
export class AddFormCategoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.data.action=="edit"){
      ...
      this.dynamicFormCategoryAdd.controls['title'].setValue(this.data?.element?.title)
    }
  }
  onSubmit() {
 ...
 if(this.data.action==="new"){
    this.dynamicService
      ...
      .subscribe(
        (res) => {
          ...
          this.router.navigate(['/admin/siteInfo/dynamicFormCategories']);
        },
        (err) => {
          console.error(err);
        }
    ...
    this.dynamicService
    .formCategoriesPut(this.data.element._id,this.dynamicFormCategoryAdd.value)
    .subscribe(
      (res) => {
        console.log('addCustomerForm=>', res);
